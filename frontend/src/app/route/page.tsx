...
function RoutePageContent() {
  ...
  const startAmbTracking = useCallback(() => {
    if (!navigator.geolocation) {
      setGeoError(t("route.noGeolocation"));
      return;
    }
  ...
  if (!status) {
    return (
      ...
      >
        ...
        <p className="text-gray-600">{t("route.loadingTask")}</p>
      </motion.div>
    );
  }
    ...
    >
      ...
      >
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">{t("route.heading")}</h1>
            <p className="text-sm text-gray-500">{t("route.taskId")} {taskId.slice(0, 8)}...</p>
          </div>
        </div>
      </motion.div>
      ...
      {status.status === "completed" && (status.patient_name || status.patient_mobile) && (
        ...
        >
          <div className="flex items-center gap-2 mb-3">
            ...
            <div>
              <h3 className="text-sm font-bold text-gray-800">{t("route.patientInfo")}</h3>
              <p className="text-xs text-gray-500">{t("route.patientSubtitle")}</p>
            </div>
          </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-6 gap-3">
                {status.patient_uhid && (
                  <div className="bg-white/60 rounded-xl p-3">
                    <p className="text-xs text-gray-500 font-medium">{t("route.uhid")}</p>
                    <p className="text-sm font-bold text-gray-900 font-mono">{status.patient_uhid}</p>
                  </div>
                )}
                {status.patient_date && (
                  <div className="bg-white/60 rounded-xl p-3">
                    <p className="text-xs text-gray-500 font-medium">{t("route.date")}</p>
                    <p className="text-sm font-bold text-gray-900">{status.patient_date}</p>
                  </div>
                )}
                {status.patient_case && (
                  <div className="bg-red-50 rounded-xl p-3 border border-red-200">
                    <p className="text-xs text-red-600 font-medium">{t("route.caseType")}</p>
                    <p className="text-sm font-bold text-red-800">{status.patient_case}</p>
                  </div>
                )}
                {status.patient_name && (
                  <div className="bg-white/60 rounded-xl p-3">
                    <p className="text-xs text-gray-500 font-medium">{t("route.patientName")}</p>
                    <p className="text-sm font-bold text-gray-900 truncate">{status.patient_name}</p>
                  </div>
                )}
                {status.patient_age && (
                  <div className="bg-white/60 rounded-xl p-3">
                    <p className="text-xs text-gray-500 font-medium">{t("route.age")}</p>
                    <p className="text-sm font-bold text-gray-900">{status.patient_age} {t("route.years")}</p>
                  </div>
                )}
                {status.patient_sex && (
                  <div className="bg-white/60 rounded-xl p-3">
                    <p className="text-xs text-gray-500 font-medium">{t("route.sex")}</p>
                    <p className="text-sm font-bold text-gray-900">{status.patient_sex}</p>
                  </div>
                )}
                {status.patient_blood_group && (
                  <div className="bg-red-50 rounded-xl p-3 border border-red-200">
                    <p className="text-xs text-red-600 font-medium">{t("route.bloodGroup")}</p>
                    <p className="text-sm font-bold text-red-800">{status.patient_blood_group}</p>
                  </div>
                )}
                {status.patient_mobile && (
                  <div className="bg-white/60 rounded-xl p-3">
                    <p className="text-xs text-gray-500 font-medium">{t("route.mobile")}</p>
              ...
              {(status.patient_bp_systolic || status.patient_bp_diastolic ||
                ...
                <div className="mt-4 pt-4 border-t border-orange-200/50">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-xs font-bold text-cyan-700">{t("route.vitals")}</span>
                  </div>
                  <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
                    {(status.patient_bp_systolic || status.patient_bp_diastolic) && (
                      <div className="bg-cyan-50 rounded-xl p-3 border border-cyan-200">
                        <p className="text-xs text-cyan-600 font-medium">{t("route.bp")}</p>
                        <p className="text-sm font-bold text-cyan-800">
                          {status.patient_bp_systolic}/{status.patient_bp_diastolic} {t("route.bpUnit")}
                        </p>
                      </div>
                    )}
                    {status.patient_temperature && (
                      <div className="bg-orange-50 rounded-xl p-3 border border-orange-200">
                        <p className="text-xs text-orange-600 font-medium">{t("route.temperature")}</p>
                        <p className="text-sm font-bold text-orange-800">{status.patient_temperature}{t("route.tempUnit")}</p>
                      </div>
                    )}
                    {status.patient_pulse && (
                      <div className="bg-rose-50 rounded-xl p-3 border border-rose-200">
                        <p className="text-xs text-rose-600 font-medium">{t("route.pulse")}</p>
                        <p className="text-sm font-bold text-rose-800">{status.patient_pulse} {t("route.pulseUnit")}</p>
                      </div>
                    )}
                    {status.patient_spo2 && (
                      <div className="bg-purple-50 rounded-xl p-3 border border-purple-200">
                        <p className="text-xs text-purple-600 font-medium">{t("route.spo2")}</p>
                        <p className="text-sm font-bold text-purple-800">{status.patient_spo2}{t("route.spo2Unit")}</p>
                      </div>
                    )}
                  </div>
                </div>
        ...
        >
            ...
            <div className="px-6 py-4 border-b border-gray-100">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-lg font-semibold text-gray-900">{t("route.map")}</h2>
                  <span className="text-sm text-gray-500 ml-2">-
                    {viewMode === 'main' ? t("route.mapMain") :
                     viewMode === 'nodes' ? t("route.mapNodes") :
                     viewMode === 'active-sensors' ? t("route.mapActive") :
                     viewMode === 'traffic-signals' ? t("route.mapTraffic") :
                     t("route.mapAmb")}
                  </span>
                </div>
          ...
          <div className="mt-4 grid grid-cols-2 gap-4">
            <div className="bg-blue-50 border border-blue-200 rounded-xl px-4 py-3 text-center">
              ...
              <div className="text-xs text-blue-600">{t("route.statsNodes")}</div>
            </div>
            <div className="bg-orange-50 border border-orange-200 rounded-xl px-4 py-3 text-center">
              ...
              <div className="text-xs text-orange-600">{t("route.statsTurns")}</div>
            </div>
          </div>
        </motion.div>
      )}
            ...
            <div className="px-6 py-4 border-b border-gray-100">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-lg font-semibold text-gray-900">{t("route.ambLocation")}</h2>
                  <span className="text-sm text-gray-500 ml-2">
                    - {roadSensors.length} {t("route.ambSensorsCount")}
                  </span>
                </div>
                <motion.button
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                  ...
                  {currentLat !== null && currentLon !== null ? (
                    <div className="space-y-3">
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <p className="text-xs text-gray-500 font-medium">{t("route.lat")}</p>
                          <p className="text-base font-bold text-gray-900">{currentLat.toFixed(6)}</p>
                        </div>
                        <div>
                          <p className="text-xs text-gray-500 font-medium">{t("route.lon")}</p>
                      ...
                      <div className="flex items-center gap-2 text-xs text-gray-500">
                        ...
                        {ambActive ? t("route.trackingLive") : t("route.trackingStopped")}
                      </div>
                    </div>
                  ) : (
                    <div className="text-center py-6">
                      ...
                      <p className="text-sm text-gray-500">
                        {ambActive ? t("route.acquiring") : t("route.clickStart")}
                      </p>
                    </div>
                  )}
                </div>
                ...
                <div className="bg-gray-50 rounded-xl p-4">
                  <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3 flex items-center gap-2">
                    ...
                    {t("route.nearestSensor")}
                  </h3>
                  {nearest ? (
                    <div className="space-y-3">
                      <div className="bg-green-50 rounded-lg p-3">
                        <div className="mb-2">
                          <p className="text-xs text-green-600 font-medium">{t("route.sensorNumber")}</p>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div>
                            <p className="text-xs text-green-600 font-medium">{t("route.sensorDistance")}</p>
                            <p className="text-base font-bold text-green-900">
                              {nearest.distance_from_amb.toFixed(3)} km
                            </p>
                          </div>
                          <div>
                            <p className="text-xs text-green-600 font-medium">{t("route.sensorPosition")}</p>
                            <p className="text-base font-bold text-green-900">
                      ...
                      {nearest.sensor.road_name && (
                        <p className="text-xs text-gray-600">
                          <span className="font-medium">{t("route.sensorRoad")}</span> {nearest.sensor.road_name}
                        </p>
                      )}
                    </div>
                  ) : (
                    <div className="text-center py-6">
                      ...
                      <p className="text-sm text-gray-500">
                        {ambActive ? t("route.calculatingSensor") : t("route.waitingLocation")}
                      </p>
                    </div>
                  )}
                </div>
                ...
                <div className={`rounded-xl p-3 flex items-center gap-3 mb-4 ${
                  ...
                  {publishStatus === "sending" && (
                    <>
                      ...
                      <p className="text-sm text-blue-700">{t("route.sendingToDevice")}</p>
                    </>
                  )}
                  {publishStatus === "sent" && (
                    <>
                      ...
                      <div>
                        <p className="text-sm text-green-700 font-medium">{t("route.dataSent")}</p>
                        {lastPublishTime && (
                          <p className="text-xs text-green-600">{t("route.lastSentAt")} {lastPublishTime}</p>
                        )}
                      </div>
                    </>
                  )}
                  {publishStatus === "error" && (
                    <>
                      ...
                      <p className="text-sm text-red-700">{t("route.sendFailed")}</p>
                    </>
                  )}
                </div>
              )}
                  ...
                  <div className="max-h-48 overflow-y-auto border border-gray-100 rounded-xl">
                    <table className="w-full text-sm">
                      <thead className="bg-gray-50 sticky top-0">
                        <tr>
                          <th className="px-4 py-2 text-left text-xs font-semibold text-gray-500 uppercase">{t("route.tableNum")}</th>
                          <th className="px-4 py-2 text-left text-xs font-semibold text-gray-500 uppercase">{t("route.tableSensorId")}</th>
                          <th className="px-4 py-2 text-left text-xs font-semibold text-gray-500 uppercase">{t("route.tableCoords")}</th>
                          <th className="px-4 py-2 text-left text-xs font-semibold text-gray-500 uppercase">{t("route.tableRoad")}</th>
                          <th className="px-4 py-2 text-left text-xs font-semibold text-gray-500 uppercase">{t("route.tableDist")}</th>
                        </tr>
                      </thead>
