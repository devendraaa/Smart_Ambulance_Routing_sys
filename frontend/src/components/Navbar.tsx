...
const navLinks = [
  ...
  { href: "/route", labelKey: "nav.route", icon: "🗺️" },
  { href: "/hospitals", labelKey: "nav.hospitals", icon: "🏥" },
  { href: "/blood-bank", labelKey: "nav.bloodBank", icon: "🩸" },
];

const baseSensorLinks = [
  { href: "/sensor", labelKey: "nav.sensorManagement", icon: "📡" },
  { href: "/sensor-map", labelKey: "nav.sensorMap", icon: "🗺️" },
  { href: "/installed-sensors", labelKey: "nav.installedSensors", icon: "📍" },
];
...
function DriverNavbar() {
  ...
  const sensorLinks = [
    ...baseSensorLinks,
    { href: ambLocationLink, labelKey: "nav.ambLocation", icon: "📍" },
  ];
        ...
        <div className="flex items-center justify-between h-16 md:h-20">
          ...
          <div className="hidden md:flex items-center gap-1">
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              >
                <span className="hidden lg:inline">{t("nav.sensors")}</span>
        ...
        <AnimatePresence>
          {mobileMenuOpen && (
            ...
            >
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mt-4">
                {navLinks.map((link) => (
                  <Link key={link.href} href={link.href} onClick={() => setMobileMenuOpen(false)}
                    ...
                    <span className="text-xs font-medium leading-tight text-center">{t(link.labelKey)}</span>
                  </Link>
                ))}
                {sensorLinks.map((link) => (
                  <Link key={link.href} href={link.href} onClick={() => setMobileMenuOpen(false)}
              ...
              <div className="mt-4 pt-4 border-t border-gray-200/50 flex items-center justify-between px-2">
                <LanguageSwitcher theme="minimal" />
                {loggedIn ? (
                  ...
                  <Link href="/login" onClick={() => setMobileMenuOpen(false)}
                    className="px-5 py-2.5 text-sm font-medium text-white bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-700 rounded-xl">
                    {t("auth.login")}
                  </Link>
                )}
              </div>
